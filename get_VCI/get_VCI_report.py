...
def get_VCI_report(x):
    url = f'https://www.vietcap.com.vn/api/cms-service/v1/page/analysis?is-all=true&page=0&size={x}&direction=DESC&sortBy=date&language=2'
    r = requests.get(url)
...
while True:
    try:
        num = int(input('Enter the number of report you wanna crawl: '))
        get_VCI_report(num)
        break
    except:
